...
public class SemanticKernelNarrationService implements NarrationService {
    ...
    public String narrate(Fight fight) throws Exception {
        ...
        TextCompletion textCompletion = SKBuilders.chatCompletion().withOpenAIClient(client)
                .withModelId("gpt-3.5-turbo").build();

        Kernel kernel = SKBuilders.kernel().withDefaultAIService(textCompletion).build();

        ReadOnlyFunctionCollection skill = kernel.importSkillFromResources("",
                "NarrationSkill", "NarrationFight");
        CompletionSKFunction fightFunction = skill.getFunction("NarrationFight", CompletionSKFunction.class);
